...
rt2 <- analytic_data %>% 
  group_by(Race) %>% 
  summarise(
    New.cases.cancer.EP = sum(is_EP),
    New.cases.cancer = n(),
    EP.rate = round(mean(is_EP) * 100, 1)
  ) %>% 
  rename(Characteristic = Race) %>% 
  mutate(factor = "Race") %>% 
  select(factor, Characteristic, New.cases.cancer, New.cases.cancer.EP, EP.rate) %>% 
  arrange(desc(New.cases.cancer)) 

at2 <- analytic_data %>%
  mutate(Characteristic = as.character(ntile(Age_yrs_at_dx, 4))) %>% 
...
At the hospital network level, the EP rate for patients newly diagnosed with colorectal cancer was `r eprate`%  (`r numer` / `r denom` cancer diagnoses). Table 1 outlines the characteristics of all patients with incident colorectal cancer and the subset with EP. 
